 ...
 * Parses trading history from MT4/MT5 HTML export
 */
    ...
    for (const table of Array.from(tables)) {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (headerText.includes('Ticket') && headerText.includes('Open Time')) {
          // This is the header row, next rows are trades
          ...
          break; // Found the trades table, no need to continue
        }
      }
    }
